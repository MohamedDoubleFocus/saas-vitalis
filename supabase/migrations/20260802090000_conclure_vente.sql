-- ============================================================================
-- saas-vitalis — conclure_vente() (module 3, le closer)
--
-- POURQUOI cette fonction existe.
--
-- Conclure une vente écrit dans QUATRE tables : `opportunites` (statut, montant,
-- dépôt, dates cibles, infos client), `opportunite_travaux` (les volets),
-- `extras` et `notes` (piste d'audit). Depuis le navigateur, PostgREST ne sait
-- pas enchaîner ça dans une transaction : une coupure réseau au milieu — un
-- closer dans un sous-sol — laisserait une vente à moitié écrite, avec des
-- volets sans opportunité vendue, ou l'inverse. Ici, c'est tout ou rien.
--
-- Second rôle : la validation SERVEUR. Une vérification faite dans le formulaire
-- n'est pas une validation ; celle-ci l'est.
--
-- ⚠️ `security invoker` (le défaut, rendu explicite) : la fonction s'exécute avec
-- les droits de l'appelant, donc TOUTES les politiques du module 1 s'appliquent
-- à l'intérieur. Un closer ne peut conclure que les rendez-vous qui lui sont
-- assignés — ce n'est pas cette fonction qui l'autorise, c'est la RLS.
--
...
-- 1. LA FONCTION
-- ----------------------------------------------------------------------------
-- Les volets et les extras arrivent en JSONB : leur nombre est variable et
-- Postgres n'a pas de type tableau d'enregistrements pratique à traverser
-- depuis PostgREST.
--
--   p_volets : [{ "type": "traitement_gonano", "produit_gonano": "fortify",
--                 "deuxieme_couche_fortify": true, "montant": 2400 }, …]
--   p_extras : [{ "description": "Gouttières", "montant": 850 }, …]
--
-- Retourne le montant des volets (ce qui est écrit dans `montant_contrat`).
-- ============================================================================
...
-- 2. DROITS
-- ----------------------------------------------------------------------------
-- `authenticated` suffit : la fonction vérifie elle-même le rôle, et la RLS
-- filtre les lignes. `anon` n'a rien à faire ici.
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- Aucune table, aucune colonne, aucune politique. Les écritures de la fonction
-- passent exactement par les politiques du module 1 :
--   • `opportunites_update_closer`  → la mise à jour de l'opportunité
--   • `peut_modifier_opportunite()` → les volets, les extras et la note
-- Si un jour ces politiques changent, la fonction suit automatiquement.
-- ============================================================================
